...
<!--                     </div> -->
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="tabMntProdBaseRel" cellspacing="0" width="100%">
                            <thead>
                                <tr>
...
<script>
    $(function() {
        ...
        var tableObj = $('#tabMntProdBaseRel').DataTable(getConfig());
        $('#btnQuery').click(function(e) {
            var strId = $("#id").val();
            var strProdId = $("#prodId").val();
            var strBaseId = $("#baseId").val();
            var sUrl = "inst/prodbase/query?";
            sUrl += 
                "&id=" + strId +
                "&prodId=" + strProdId +
                "&baseId=" + strBaseId;
        ...
        $('.reload').on('click', function() {
            ...
            $('#prodId').val('');
            $('#baseId').val('');
            tableObj.ajax.url('inst/prodbase/query?id=-1').load();
        });
        
        //修改按钮
        $('.div_right').on('click', '.dt-buttons>#btnUpdate',function() {
            var checkedBox = $("#tabMntProdBaseRel tbody tr td .checked");
            ...
            showModal(2, " ", [ '1000px', '500px' ], "inst/prodbase/update_page/" + strId, tableObj);
        });
        
        //新增
        $('.div_right').on('click', '.dt-buttons>#btnAdd', function(){
            showModal(2, " ", ['1000px', '500px'], "inst/prodbase/add_page", tableObj);
        });
        
        //删除按钮
        $('.div_right').on('click', '.dt-buttons>#btnDelete',function() {
            var checkedBox = $("#tabMntProdBaseRel tbody tr td .checked");
            comfirmDeleteData(tableObj, checkedBox, 'inst/prodbase/delete/');
        });
