                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-8">
                                        <input type="text" class="form-control" id="libRelHisId">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="control-label col-md-4">关联库文件编号</label>
                                    <div class="col-md-8">
...
<!--                     </div> -->
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="tabMntLibBaseRel" cellspacing="0" width="100%">
                            <thead>
                                <tr>
...
<script>
    $(function() {
        ...
        var tableObj = $('#tabMntLibBaseRel').DataTable(getConfig());
        $('#btnQuery').click(function(e) {
            var strId = $("#id").val();
            var strLibRelHisId = $("#libRelHisId").val();
            ...
            var sUrl = "product/lib_base/query?";
            sUrl += 
                "&id=" + strId +
                "&libRelHisId=" + strLibRelHisId +
                "&baseId=" + strBaseId +
                "&relDesc=" + strRelDesc +
        ...
        $('.reload').on('click', function() {
            ...
            $('#modifier').val('');
            $('#modifyDate').val('');
            tableObj.ajax.url('product/lib_base/query?id=-1').load();
        });
        
        //修改按钮
        $('.div_right').on('click', '.dt-buttons>#btnUpdate',function() {
            var checkedBox = $("#tabMntLibBaseRel tbody tr td .checked");
            ...
            showModal(2, "修改发布信息", [ '650px', '500px' ], "product/lib_base/update_page/" + strId, tableObj);
        });
        
        //新增
        $('.div_right').on('click', '.dt-buttons>#btnAdd', function(){
            showModal(2, " ", ['650px', '500px'], "product/lib_base/add_page", tableObj);
        });
        ...
        $('.div_right').on('click', '.dt-buttons>#btnDelete',function() {
            ...
            $.ajax({
                url: 'product/lib_base/delete/' + idAry,
                dataType: 'json',
                async:false,
    ...
    function getConfig() {
        return config = {
            ...
            "serverSide" : false,
            "ajax" : {
                "url" : "product/lib_base/query?id=-1"
            //    "async": false
            },
            "columns" : [ {
                "data" : "id"
