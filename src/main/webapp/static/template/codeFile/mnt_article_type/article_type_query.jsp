                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-8">
                                        <input type="text" class="form-control" id="typeCn">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="control-label col-md-4">上级分类</label>
                                    <div class="col-md-8">
                                        <input type="text" class="form-control" id="parentTypeId">
                                    </div>
                                </div>
                            </div>
                        </div>
...
<!--                     </div> -->
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="tabMntArticleType" cellspacing="0" width="100%">
                            <thead>
                                <tr>
...
<script>
    $(function() {
        ...
        var tableObj = $('#tabMntArticleType').DataTable(getConfig());
        $('#btnQuery').click(function(e) {
            var strTypeId = $("#typeId").val();
            var strTypeCn = $("#typeCn").val();
            var strParentTypeId = $("#parentTypeId").val();
            var sUrl = "article/type/query?";
            sUrl += 
                "&typeId=" + strTypeId +
                "&typeCn=" + strTypeCn +
                "&parentTypeId=" + strParentTypeId;
        ...
        $('.reload').on('click', function() {
            ...
            $('#typeCn').val('');
            $('#parentTypeId').val('');
            tableObj.ajax.url('article/type/query?typeId=-1').load();
        });
        
        //修改按钮
        $('.div_right').on('click', '.dt-buttons>#btnUpdate',function() {
            var checkedBox = $("#tabMntArticleType tbody tr td .checked");
            ...
            showModal(2, " ", [ '1000px', '500px' ], "article/type/update_page/" + strTypeId, tableObj);
        });
        
        //新增
        $('.div_right').on('click', '.dt-buttons>#btnAdd', function(){
            showModal(2, " ", ['1000px', '500px'], "article/type/add_page", tableObj);
        });
        
        //删除按钮
        $('.div_right').on('click', '.dt-buttons>#btnDelete',function() {
            var checkedBox = $("#tabMntArticleType tbody tr td .checked");
            comfirmDeleteData(tableObj, checkedBox, 'article/type/delete/');
        });
    ...
    function getConfig() {
        return config = {
            ...
            "serverSide" : false,
            "ajax" : {
                "url" : "article/type/query?typeId=-1"
            //    "async": false
            },
            "columns" : [ {
                "data" : "typeId"
